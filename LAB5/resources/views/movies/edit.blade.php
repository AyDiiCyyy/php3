...
<body>
    <div class="container">
        ...
        <form action="{{ route('edit',['id'=>$one->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <input type="text" id="title" name="title" value="{{$one->title}}">
            ...
            <input type="text" id="author" name="intro" value="{{$one->intro}}">

            <label for="publisher">Ngày công chiếu:</label>
            <input type="date" id="publisher" name="release_date" value="{{$one->release_date}}">

            <label for="category_id">Danh mục:</label>
            <select id="category_id" name="genre_id">
                <option value="">Chọn danh mục</option>
                @foreach ($listCate as $cate )
                <option value="{{$cate->id}}" {{($cate->id==$one->genre_id)?'selected':''}} >{{$cate->name}}</option>
                @endforeach
                <!-- Thêm các danh mục khác nếu cần -->
            </select>

            <button type="submit">Sửa Phim</button>
